...
public class Ensyu09_01 {
	...
	static Human[] getHumanArray () {
		...
		//配列xを用意
		Human[] humanArray = new Human[cLength];

		//一次元配列xの要素数を定義
		humanArray = new Human[cLength];
		...
		for (int idxItr = 0; idxItr < cLength; idxItr++) {
			...
			//入力値を該当要素の値として設定
			humanArray[idxItr] = new Human(name, height, weight);
			System.out.println("");
		}
		//返却値は配列；
		return humanArray;
	 ...
	 * @param humanArray
	 */
	static void printArray (Human[] humanArray) {
		for (int i = 0; i < humanArray.length; i++) {
			humanArray[i].getProf();
			//要素間は改行
			System.out.println("");
		}
	}
