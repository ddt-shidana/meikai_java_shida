 ...
 * 	また、2次元配列の表示では、各列の数値の戦闘が揃うように、最低限のスペースを空けること。<br>
 *
 * 作成日:2022/03/09<br>
 * 修正日:2023/03/16<br>
 * ver:1.1.0
...
public class Ensyu07_33 {
	 ...
	 * @return
	 */
	private static int valOfLength (int[][] a, int l, int c) {
		int valOfLength = -1;
		if (a[l].length <= c) {
			valOfLength = 0;
		}
	 ...
	 * @return
	 */
	private static int[] valOfMaxLength (int[][] a) {
		//各行の列数を格納する配列を取得
	...
	/**
	 * 二次元配列の要素を各列の数値の先頭が揃うように表示する
	 * @param a
	 */
	private static void printValArray (int[][] a) {
		//各行
		for (int lItr = 0; lItr < a.length; lItr++) {
			//見やすいようにタブ
			System.out.print("\t");
