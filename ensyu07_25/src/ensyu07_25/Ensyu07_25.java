...
public class Ensyu07_25 {
	...
	public static void main(String[] args) {
		...
		do {
			...
			System.out.print("b");
			//中身からフッダまでの表示をメソッド呼び出し
			printArray(removeArrayIndexs(array, idxIn, nIn));
	 ...
	 * @return 削除後の配列
	 */
	private static int[] removeArrayIndexs(int[] a, int idx, int n) {
		//求める配列、要素数は引数の長さから削除するnを引いた数
		int[] removeArrayIndexs = new int[a.length - n];
		//idxが0より大きい時
		if (idx > 0) {
			//idxより前のインデックスの要素を求める配列に格納
			for (int placeItr = 0; placeItr < idx; placeItr++) {
				removeArrayIndexs[placeItr] = a[placeItr];
			}
		}
		//idx+nがaの要素の長さより小さい時
		if (idx + n < a.length) {
			...
			for (int placeItr = idx; placeItr < idx + times; placeItr++) {
				removeArrayIndexs[placeItr] = a[placeItr + n];
			}
		}
		//返却値は配列
		return removeArrayIndexs;
	}
